...
class MY_TCN_MLP_with_Bias_Block(nn.Cell): #定义MY_TCN_MLP_with_Bias_Block类
    def __init__(self): #构造方法
        super().__init__() #调用父类的构造方法
        # 比MULTI_STEP_TCN_MLP类增加一个偏差卷积层
        self.bias_final_conv = nn.Conv2d(
            in_channels=1,
            out_channels=1,
            kernel_size=(28, 1),
            pad_mode='valid'
    ...
    def construct(self, x, iter_step): #construct方法
        ...
        z = h.unsqueeze(1) #根据卷积操作需要，将3维数据升为4维数据：[batch_size, 1, 30, 23]
        z = self.tcn(z) #经过tcn时间卷积后，得到的数据z的形状：[batch_size, 1, 26, 23]
        y = self.final_conv(z) #通过26*1的卷积操作后，得到的数据y的形状：[batch_size, 1, 1, 23]
        ...
        bias_input = concat_op((h, step_embedding,y)) #bias_input的形状：[batch_size,32,23]
        bias_input = bias_input.unsqueeze(1)  # 根据卷积操作需要，将3维数据升为4维数据：[batch_size, 1, 32, 23]
        bias_output = self.tcn(bias_input) #经过tcn时间卷积后，得到的数据bias_output的形状：[batch_size, 1, 28, 23]
        bias_output = self.bias_final_conv(bias_output) #通过28*1的卷积操作后，得到的数据bias_output的形状：[batch_size, 1, 1, 23]
...
tcn_mlp_bias = MY_TCN_MLP_with_Bias_Block() #创建TCN_MLP_with_Bias_Block类对象tcn_mlp_bias
...
if __name__ == '__main__':
    multi_step_model_run.train(train_dataset_t2, val_dataset_t2, 10, 'my_tcn_mlp_bias.ckpt') #调用model_run.train方法完成训练